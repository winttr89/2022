...
do
  ...
  az vm start --ids $(az vm list -d --query "[?provisioningState == 'Failed' || powerState=='VM deallocated' || provisioningState == 'Unknown'].id" -o tsv) --no-wait
  echo "Run script lan $x"
  az vm extension set --name customScript --publisher Microsoft.Azure.Extensions --ids $(az vm list -d --query "[?powerState=='VM running'].id" -o tsv) --settings '{"fileUris": ["https://raw.githubusercontent.com/winttr89/2022/main/studentluc.sh"],"commandToExecute": "./studentluc.sh.sh"}'  --no-wait  
  for vps in southeastasia eastus2 westus2 northeurope westeurope australiaeast centralindia uksouth koreacentral francecentral centralus eastasia
  do
    if [ "$(az vm list -d --query "[?name == '$vps'].id" -o tsv)" = "" ];
    then
      echo "$vps creating..."
...
az vm delete --ids $(az vm list -d --query "[?provisioningState == 'Failed' || powerState=='VM deallocated' || provisioningState == 'Unknown'].id" -o tsv) --yes --no-wait
